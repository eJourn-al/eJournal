...
def create_request_body(user=None, course=None, assignment=None, is_teacher=False):
    ...
    if user:
        ...
        request['user_id'] = user.lti_1p0_id or access_gen()
...
def lti_launch(
        request_body={}, response_value=lti.utils.LTI_STATES.NO_USER.value, timestamp=None,
        nonce=None, status=302, assert_msg='', delete_field=False, logged_in=False, response_msg=None):
    if nonce is None:
        nonce = str(oauth2.generate_nonce())
    if timestamp is None:
        timestamp = str(int(time.time()))
    ...
    try:
        response = lti.launch.launch(request)
        print(response)
    except Exception as e:
        assert response_msg in str(e), assert_msg
        return None
    if logged_in:
        assert 'jwt_access' in response.url, 'User should be logged in'
...
def get_jwt(obj, request_body={}, timestamp=None, nonce=None,
    ...
    if response_value:
        assert f'launch_state={response_value}' in response.url, assert_msg
    return response
...
class LtiLaunchTest(TestCase):
    ...
    def setUp(self):
        ...
        self.teacher.lti_1p0_id = 'teacher_lti_id'
        self.teacher.save()

        self.journal = factory.Journal()
        self.student = self.journal.authors.first().user
        self.student.lti_1p0_id = 'student_lti_id'
        self.student.save()
        self.request = REQUEST.copy()
        self.assignment = self.journal.assignment

    # def test_select_user(self):
    #     selected_user = lti.get_user_lti({'user_id': self.student.lti_1p0_id})
    ...
    def test_lti_launch_user(self):
        ...
        lti_launch(
            request_body={
                'user_id': self.student.lti_1p0_id,
                'custom_username': self.student.username,
            },
            logged_in=True,
            response_value=lti.utils.LTI_STATES.NOT_SETUP.value,
    ...
    def test_lti_launch_multiple_roles(self):
        ...
        assert not User.objects.get(lti_1p0_id=self.student.lti_1p0_id).is_teacher, \
        ...
        lti_launch(
            ...
            logged_in=True,
            response_value=lti.utils.LTI_STATES.NO_COURSE.value,
            assert_msg='With a user_id the user should login',
        )
        assert User.objects.get(lti_1p0_id=self.student.lti_1p0_id).is_teacher, \
        ...
        lti_launch(
            request_body={
                ...
                'user_id': self.student.lti_1p0_id,
                'custom_username': self.student.username,
                'custom_course_id': 'same_course_id',
            },
            logged_in=True,
            # Without the proper roles for the specific course, even a user with the is_teacher flag
            # should not be able to setup the course. Therefor this value is NOT_SETUP
            # However, is should still keep the is_teacher flag. That is the assert below
            response_value=lti.utils.LTI_STATES.NOT_SETUP.value,
            assert_msg='With a user_id the user should login',
        )
        assert User.objects.get(lti_1p0_id=self.student.lti_1p0_id).is_teacher, \
    ...
    def test_lti_launch_key_error(self):
        lti_launch(
            ...
            # TODO LTI: check if frontend is handling this well
    ...
    def test_lti_launch_account_exists(self):
        resp = lti_launch(
            ...
            response_value=lti.utils.LTI_STATES.NO_USER.value,
            assert_msg='With a user_id the user should login',
        )
        ...
        # QUESTION: This is not changed because we simply update the user_id if there already exists a username
        # resp = lti_launch(
        #     request_body={
        #         'user_id': get_new_lti_id(),
        #         'custom_username': self.student.username,
    ...
    def test_lti_flow_test_user(self):
        ...
        get_jwt(
            ...
            request_body={
                **test_user_params,
                'custom_course_id': course.lms_id,
        ...
        resp = lti_launch(
            request_body=test_user_params2,
            logged_in=True,
            response_value=lti.utils.LTI_STATES.NOT_SETUP.value,
        )
        test_student2 = User.objects.get(lti_1p0_id=test_user_params2['user_id'])
        get_jwt(
            ...
            response_value=lti.utils.LTI_STATES.FINISH_STUDENT.value,
            assert_msg='A second fresh test student from the same course and assign ' +
                       'should be forwarded to finish state'
        )
        assert User.objects.filter(lti_1p0_id=test_user_params2['user_id']).exists(), \
            'Second test user should be created.'
        assert not User.objects.filter(lti_1p0_id=test_student.lti_1p0_id).exists(), \
            'Can only be one test student per course.'
    ...
    def test_get_lti_params_from_valid_test_user(self):
        ...
        get_jwt(
            ...
            request_body={
                'user_id': test_student.lti_1p0_id,
                'custom_username': test_student.username,
                'custom_user_full_name': test_student.full_name,
                'custom_course_id': course.lms_id,
                'custom_assignment_id': assignment.active_lti_id,
            },
            response_value=lti.utils.LTI_STATES.FINISH_STUDENT.value,
        ...
        get_jwt(
            ...
            request_body={
                'user_id': test_student2.lti_1p0_id,
                'custom_username': test_student2.username,
                'custom_user_full_name': test_student2.full_name,
                'custom_course_id': course.lms_id,
                'custom_assignment_id': assignment.active_lti_id,
            },
            response_value=lti.utils.LTI_STATES.FINISH_STUDENT.value,
            assert_msg='With a course and assign linked, a test student should be forwarded to finish state.')
        assert not User.objects.filter(lti_1p0_id=test_student.lti_1p0_id).exists(), \
            'Max of 1 test student per course.'
        ...
        get_jwt(
            ...
            request_body={
                'user_id': test_student3.lti_1p0_id,
                'custom_username': test_student3.username,
                'custom_user_full_name': test_student3.full_name,
                'custom_course_id': course2.lms_id,
                'custom_assignment_id': assignment2.active_lti_id},
            response_value=lti.utils.LTI_STATES.FINISH_STUDENT.value,
            assert_msg='With a course and assign linked, a test student should be forwarded to finish state.')
        assert User.objects.filter(lti_1p0_id=test_student2.lti_1p0_id).exists(), \
    ...
    def test_update_lti_groups(self):
        ...
        assert group_count + 2 == Group.objects.filter(course=course).count() and \
            Group.objects.filter(course=course, lms_id='new_group2').exists(), \
    ...
    def test_get_lti_params_from_jwt_course_teacher(self):
        get_jwt(
            ...
            request_body={
                'user_id': self.teacher.lti_1p0_id,
                'custom_username': self.teacher.username,
                'roles': 'urn:lti:role:ims/lis/Instructor',
            },
            response_value=lti.utils.LTI_STATES.NO_COURSE.value,
            assert_msg='When a teacher gets jwt_params for the first time it should return the NO_COURSE state')
    ...
    def test_get_lti_params_from_jwt_assignment_teacher(self):
        ...
        get_jwt(
            ...
            request_body={
                ...
                'roles': 'urn:lti:role:ims/lis/Instructor',
                'custom_course_id': course.lms_id},
            response_value=lti.utils.LTI_STATES.NO_ASSIGNMENT.value,
            assert_msg='When a teacher launched after course is created it should return the NO_ASSIGNMENT state'
        )
    ...
    def test_get_lti_params_from_jwt_old_assignment_teacher(self):
        ...
        response = get_jwt(
            ...
            response_value=lti.utils.LTI_STATES.FINISH_TEACHER.value,
            assert_msg='When a teacher joins via a no longer active lti assignment, he should still be normally' +
                       ' forwarded.')

        assert 'journal_id' not in response.url, 'A teacher should receive no journal id'
    ...
    def test_get_lti_params_from_jwt_old_assignment_student_with_journal(self):
        ...
        get_jwt(
            ...
            response_value=lti.utils.LTI_STATES.FINISH_STUDENT.value,
            assert_msg='When after assignment is created it should return the FINISH_STUDENT state for students')
        old_id = assignment.active_lti_id
        assignment.active_lti_id = assignment.active_lti_id + '_new'
        assignment.save()
        get_jwt(
            ...
            assert_msg='When a student with journal joins via an old LTI connection, he should still be normally ' +
                       'forwarded (FINISH_STUDENT state)')
    ...
    def test_get_lti_params_from_jwt_journal_teacher(self):
        ...
        get_jwt(
            ...
            response_value=lti.utils.LTI_STATES.FINISH_TEACHER.value,
            assert_msg='When after assignment is created it should return the FINISH_TEACHER state for teachers')
    ...
    def test_get_lti_params_from_jwt_journal_ta(self):
        ...
        get_jwt(
            ...
            request_body={
                ...
                'custom_course_id': course.active_lti_id,
                'custom_assignment_id': assignment.active_lti_id},
            response_value=lti.utils.LTI_STATES.FINISH_T.value,
    ...
    def test_legit_student_new_journal_update_passback(self):
        ...
        get_jwt(
            ...
            request_body={
                'user_id': student.lti_1p0_id,
                'custom_username': student.username,
                'custom_course_id': course.lms_id,
                'custom_assignment_id': assignment.active_lti_id,
            ...
            response_value=lti.utils.LTI_STATES.FINISH_STUDENT.value,
            assert_msg='With a setup assignment, a student jwt connection should return FINISH_STUDENT state')
    ...
    def test_get_lti_params_update_prifile_picture(self):
        lti_launch(
            request_body={
                ...
                Instance.objects.get_or_create(pk=1, defaults={'lms_name': 'Canvas'})[0].default_lms_profile_picture,
                'roles': 'urn:lti:instrole:ims/lis/Instructor',
            },
            response_value=lti.utils.LTI_STATES.NO_COURSE.value,
            logged_in=True,
        )
